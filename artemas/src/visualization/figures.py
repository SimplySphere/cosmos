...
DARK = "#111827"
PROBABILITY_RIVER_BUILD = "probability-river-legend-4x2-v11-20260729"
METHOD_COLORS = {
...
def _save_probability_river(config: dict, paths: dict, output_dir: Path) -> list[str]:
    ...
    figure = make_subplots(
        ...
        subplot_titles=(
            "Every Monte Carlo future generated from the frozen end-of-2025 model",
            "Observed Yahoo basket minus transformer median",
        ),
    )

    # ------------------------------------------------------------------
    # BACKGROUND: all top-panel lines use Scattergl so layer order is
    # deterministic within one WebGL renderer.
    # ------------------------------------------------------------------
    figure.add_trace(
        _plotly_path_cloud_trace(
            dates,
            paths_with_origin,
            MEDIAN_BLUE,
            f"{len(simulated):,} Monte Carlo paths",
            opacity=0.035,
        ).update(legendrank=70),
        row=1,
        col=1,
    )

    for lower, upper, label, dash, rank in [
        (0, 6, "95% bounds", "dot", 60),
        (1, 5, "80% bounds", "dash", 50),
        (2, 4, "50% bounds", "solid", 40),
    ]:
        figure.add_trace(
            go.Scattergl(
                x=dates,
    ...
    if len(actual_values):
        for date, model_value, actual_value in zip(
            actual_dates, median_observed, actual_values
        ):
            connector_x.extend([date, date, None])
            connector_y.extend([model_value, actual_value, None])

        # Lower-panel bars can be added before the final foreground traces.
        ...
        # Structural shapes stay below the top-panel data.
        figure.add_vrect(
            x0=actual_dates[-1],
            x1=forecast_dates[-1],
            ...
            line_width=0,
            layer="below",
            annotation_text="Forecast-only after latest complete week",
    ...
    # ------------------------------------------------------------------
    # FOREGROUND: deliberately LAST top-panel Scattergl traces.
    # Same renderer as the 5,000-path cloud, so later traces paint on top.
    # ------------------------------------------------------------------

    # Thick median under-stroke.
    figure.add_trace(
        go.Scattergl(
            x=dates,
            y=quantiles[3],
            mode="lines",
            line={"color": "#0B4F8A", "width": 7.0},
            showlegend=False,
            hoverinfo="skip",
        ),
        row=1,
    ...
    figure.add_trace(
        go.Scattergl(
            ...
            y=quantiles[3],
            mode="lines",
            name="Transformer median",
            legendrank=20,
            line={"color": "#1769B0", "width": 4.3},
            hovertemplate="%{x|%Y-%m-%d}<br>Median: %{y:.4f}<extra></extra>",
        ),
        row=1,
        col=1,
    )
    ...
    if len(actual_values):
        # Red connectors go over the median.
        figure.add_trace(
            go.Scattergl(
                x=connector_x,
                y=connector_y,
                mode="lines",
                name="Median-to-actual gap",
                legendrank=30,
                line={"color": "rgba(214,39,40,0.98)", "width": 2.4},
                hoverinfo="skip",
            ),
            row=1,
            col=1,
        ...
        # Dark gold under-stroke.
        figure.add_trace(
            go.Scattergl(
                x=actual_dates_with_origin,
                y=actual_with_origin,
                mode="lines",
                line={"color": "#A57600", "width": 7.0},
                showlegend=False,
                hoverinfo="skip",
            ),
            row=1,
        ...
        # Bright gold actual — FINAL TOP-PANEL TRACE.
        figure.add_trace(
            go.Scattergl(
                x=actual_dates_with_origin,
                y=actual_with_origin,
                mode="lines",
                name="Observed Yahoo basket",
                legendrank=10,
                line={"color": "#E3B21B", "width": 4.6},
                hovertemplate="%{x|%Y-%m-%d}<br>Observed: %{y:.4f}<extra></extra>",
            ),
            row=1,
            col=1,
        )

        # Comparison-point dots: one marker for each observed week that is
        # directly compared with the transformer median by a red connector.
        figure.add_trace(
            go.Scattergl(
                x=actual_dates,
                y=actual_values,
    ...
    figure.update_yaxes(
        title_text="Cumulative equal-weight Yahoo basket index<br>(forecast origin = 1.0)",
        row=1,
        col=1,
    )
    ...
    figure.update_layout(
        title="Transformer + Monte Carlo Forecast Versus Observed Yahoo Finance Prices",
        template="plotly_white",
        meta={"probability_river_build": PROBABILITY_RIVER_BUILD},
        height=820,
        legend={
            "orientation": "h",
            "x": 0.5,
    ...
    # Terminal proof that this exact version generated the HTML.
    print(f"[ARTEMAS] probability river build: {PROBABILITY_RIVER_BUILD}")
    print(f"[ARTEMAS] source file: {Path(__file__).resolve()}")
    print(f"[ARTEMAS] wrote: {html_path.resolve()}")
    ...
    # Background cloud and interval boundaries.
    _add_all_paths_static(
        top,
        dates,
        paths_with_origin,
        MEDIAN_BLUE,
    ...
    if len(actual_values):
        ...
        # Gold observed path on top.
        top.plot(
            actual_dates_with_origin,
            actual_with_origin,
            color="#A57600",
            linewidth=5.6,
            zorder=40,
        ...
        lower.bar(
            ...
            gap,
            width=5.5,
            color=[ACTUAL_GOLD if value >= 0 else ERROR_RED for value in gap],
            alpha=0.85,
        )
    ...
    top.set_title(
        "All Transformer Monte Carlo Paths Versus Observed Yahoo Finance Prices\n"
...
def main() -> None:
    print(f"[ARTEMAS] executing: {Path(__file__).resolve()}")
    print(f"[ARTEMAS] build: {PROBABILITY_RIVER_BUILD}")
